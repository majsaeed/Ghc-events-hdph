...
import HdpH.ShowSparkCont ( showSparkContention )
import HdpH.ShowRegCont (showRegContention )
...
command ["show", "caps", file] = do
    ...
    putStrLn . showMap
      ...
      (unlines . map (("  " ++) . ppEvent eventTypeMap)) $
        capMap
-- This is the original merge function of Ghc-events
-- It only can merge two file so I have upgrade this function to merge
-- multiple eventlogs instead.
...
-- This the new merge function upgraded to mereg multiple eventlogs
-- instead of only merging two evetlog.

command ("merge":out:files) = do
    fs <-  mapM readLogOrDie files
    let m = foldr1 mergeEventLogs fs
    writeEventLogToFile out m

-- This function takes an HdpH eventlog and sync the events times.
command ["synch", starttime, out, file] = do
    log <- readLogOrDie file
    let stime = read starttime :: Word64
    let s = synchEventLogs stime log
    writeEventLogToFile out s

-- This function shows the startup time of ad HdpH node.
command ["showstrevt", file] = do
    log <- readLogOrDie file
    showStrEvt log

-- This function shows the HdpH events 
command ["showel", file] = do
    log <- readLogOrDie file
    showEventLog log

-- This function is the contention analysis for HdpH sparkpool
-- It reads the eventlog and gives statistical information about
-- schedulers entering the sparkpool.
command ["showsparkcont", file] = do
    log <- readLogOrDie file
    showSparkContention log

-- This function is the registry contention analysis tool for HdpH
-- it reads the eventlog and produces statistical information about
-- schedulers accessing the registry

command ["showregcont", file] = do
    log <- readLogOrDie file
    showRegContention log
