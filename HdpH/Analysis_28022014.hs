...
showSparkContention hdph_events = do
  let 
...
--TODO change the namke to pairSparkEvents
...
--   let     
  ...
  displayGroupConfType totalConflicts groupConfType durConfType
...
displayGroupReg tc [] []= do
 putStrLn "------------------------------------------------------------------------------"
displayGroupReg tc ((a,b):[]) ((_,c):[]) = do 
 let -- a = number of schedulers in a conflict
     ...
     cmd = meanDuration c'  b -- mean duration of conflicts
 printf "%-15d %-20.2f %-20.4f %-20.4f\n" a b' c' cmd
 displayGroupReg tc [] []
displayGroupReg tc ((a,b):ys) ((_,c):cs) = do 
 let -- a = number of schedulers in a conflict
     -- b = conflicts total occurance
     b' = percentage  tc b -- conflict percentage
     -- c = total conflict duration for that particuler number of schedulers
     ...
     cmd = meanDuration c'  b -- mean duration of conflicts
 printf "%-15d %-20.2f %-20.4f %-20.4f\n" a b' c' cmd
 displayGroupReg tc ys cs


displayGroupConfType tc [] [] = do
 putStrLn "------------------------------------------------------------------------------"
displayGroupConfType tc ((a,b):[]) ((_,c):[]) = do
 let -- a = events type
 ...
 printf "%-15s %-20.2f %-20.4f %-20.4f\n" a b' c' cmd 
 displayGroupConfType tc [] []
displayGroupConfType  tc ((a,b):ys) ((_,c):cs) =  do
 let -- a = events type
     -- b = number of occurance
     b' = percentage  tc b
     -- c = total duration of these conflicts
