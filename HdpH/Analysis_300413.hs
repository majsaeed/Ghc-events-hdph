...
projectSchedulerId (x:xs) = 
 case  e_spec x  of 
      CreateSpark id -> id: projectSchedulerId xs 
...
-- conflictEvents and remve the non-productive events
...
-- new version work with the new liftEvent function
getHdpHEvents (x:xs) = 
    let evt = liftEvent x
    in case  e_spec evt of
       NotHdpHEvent ->  getHdpHEvents xs
...
-- It take an Event and return it in the form HdpHEvent
proEvent :: Event -> HdpHEvent 
proEvent e | "ptest16" `isInfixOf` msg (spec e ) =
 HdpHEvent (time e) (EnterSparkPool ((read (head(words (msg (spec e))))):: Word16))
           | "ptes17" `isInfixOf` msg (spec e ) =
 HdpHEvent (time e) (NothingToSpark ((read (head(words (msg (spec e))))):: Word16))
           | "ptest02" `isInfixOf` msg (spec e ) =
 HdpHEvent (time e) (ConvertSpark ((read (head(words (msg (spec e))))):: Word16))
           | "ptest21" `isInfixOf` msg (spec e ) =
 HdpHEvent (time e) (PutSpark ((read (head(words (msg (spec e))))):: Word16))
           | "ptest22" `isInfixOf` msg (spec e ) =
 HdpHEvent (time e) (PutSpark ((read (head(words (msg (spec e))))):: Word16))
           | otherwise = HdpHEvent (time e) NotHdpHEvent
...
-- to be removed soon
-- Concat and sort the events from the sparkPoolEvent
getSPoolEvents :: Data -> [Event]
getSPoolEvents a = sort ( concat (sparkPoolEvents a))

-- to replace getSPoolEvents
...
-- to be removed soon
-- Extract the HdpH event from the eventlog 
sparkPoolEvents :: Data -> [[Event]]
sparkPoolEvents d@(Data (x:xs)) =  getBlockEvents x : sparkPoolEvents (Data xs)
sparkPoolEvents (Data []) = []

-- to replace sparkPoolEvents 
...
-- to be removed soon
-- Extract the sparkpool events form and event_block
getBlockEvents x = 

     let b_events = block_events (spec x)
         poolEvents = isPoolEvt b_events 
           where isPoolEvt (e:es) | isUserMsgEvent ( spec e) && "CKCOT" `isInfixOf` msg (spec e) =   e : isPoolEvt es
                                  | otherwise = isPoolEvt es
                 isPoolEvt [] = []

     in  poolEvents

-- to replace getBlockEvents
