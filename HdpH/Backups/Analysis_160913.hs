...
--   let     
  ...
  displayGlob  totalSchedGlob waitGlob conflictGlob
  putStrLn "Displayed times are in microseconds."
  printf "%-10s %-10s %-10s %-10s %-10s\n" "SID" "Wait Dur." "Mean" "Conf. Dur." "Mean" 
  displayDurGlob waitGlob  durWaitGlob conflictGlob durConfGlob
  ...
  displayGroupGlob groupWaitGlob groupConfGlob


  putStrLn "Registry Analysis for Dereferencing Global Reference Events"
  putStrLn "------------------------------------------------------------------------------"
  printf "%-10s %-10s %-10s %-10s %-10s %-10s\n" "SID" "Enter" "Wait" "Wait%" "Conflict" "Conflict%"   
  displayDeref  totalSchedDeref waitDeref conflictDeref 
  putStrLn "Displayed times are in microseconds."
  printf "%-10s %-10s %-10s %-10s %-10s\n" "SID" "Wait Dur." "Mean" "Conf. Dur." "Mean" 
  displayDurDeref waitDeref  durWaitDeref conflictDeref durConfDeref
  ...
  putStrLn "Number of times waitng or conflict occured with this number of schedulers"
  printf "%-15s %-20s %-20s\n" "No.Schedulers" "Wait. Occurance " "Conf. Occurance" 
  displayGroupDeref groupWaitDeref groupConfDeref
  ...
  putStrLn "------------------------------------------------------------------------------"
  printf "%-10s %-10s %-10s %-10s %-10s %-10s\n" "SID" "Enter" "Wait" "Wait%" "Conflict" "Conflict%" 
  displayFree  totalSchedFree waitFree conflictFree
  putStrLn "Displayed times are in microseconds."
  printf "%-10s %-10s %-10s %-10s %-10s\n" "SID" "Wait Dur." "Mean" "Conf. Dur." "Mean" 
  displayDurFree waitFree  durWaitFree conflictFree durConfFree
  putStr "Max waiting duration : "
  ...
  putStrLn "Number of times waitng or conflict occured with this number of schedulers"
  printf "%-15s %-20s %-20s\n" "No.Schedulers" "Wait. Occurance " "Conf. Occurance" 
  displayGroupFree groupWaitFree groupConfFree




displayGlob [] [] [] = putStrLn "------------------------------------------------------------------------------"

displayGlob ((a,b):[]) ((_,c):[]) ((_,d):[]) = do 
 let -- a  scheduler id
     -- b  total enters
 ...
 displayGlob [] [] []
displayGlob ((a,b):as) ((_,c):cs) ((_,d):ds)  = do 
 let -- a  scheduler id
     -- b  total enters
     -- c  total cause in waiting
     -- d  total involve in conflicts
     ...
     confPercentage  = percentage  b d
 printf "%-10d %-10d %-10d %-10.2f %-10d %-10.2f\n" a b c waitPercentage d confPercentage
 displayGlob as cs ds 


displayDurGlob [] [] [] [] = do 
 putStrLn "------------------------------------------------------------------------------" -- Don't change to printf or compilation error will occur.
displayDurGlob ((a,b):[]) ((_,c):[]) ((_,d):[]) ((_,e):[])= do
 ...
 displayDurGlob [] [] [] []
displayDurGlob ((a,b):as) ((_,c):cs) ((_,d):ds)((_,e):es)  = do
 let c' = pc2mcs c        -- waiting durtion
     e' = pc2mcs e        -- conflict duration
     cmd = meanDuration c'  b -- mean duration of waiting
     emd = meanDuration e'  d -- mean duration of conflict
 printf "%-10d %-10.4f %-10.4f %-10.4f %-10.4f\n" a  c' cmd e' emd
 displayDurGlob as cs ds es


displayGroupGlob [] []= do
 putStrLn "------------------------------------------------------------------------------"
displayGroupGlob((a,b):[]) ((_,c):[])= do 
 printf "%-15d %-20d %-20d \n" a b c
 displayGroupGlob [] []
displayGroupGlob ((a,b):ys) ((_,c):zs) = do 
 printf "%-15d %-20d %-20d \n" a b c
 displayGroupGlob ys zs


displayDeref [] [] [] = putStrLn "------------------------------------------------------------------------------"
displayDeref ((a,b):[]) ((_,c):[]) ((_,d):[]) = do 
 let -- a  scheduler id
     -- b  total enters
     -- c  total cause in waiting
     -- d  total involve in conflicts
 ...
 printf "%-10d %-10d %-10d %-10.2f %-10d %-10.2f\n" a b c waitPercentage d confPercentage
 displayDeref [] [] []
displayDeref ((a,b):as) ((_,c):cs) ((_,d):ds)  = do 
 let -- a  scheduler id
     -- b  total enters
     -- c  total cause in waiting
     -- d  total involve in conflicts
     ...
     confPercentage  = percentage  b d
 printf "%-10d %-10d %-10d %-10.2f %-10d %-10.2f\n" a b c waitPercentage d confPercentage
 displayDeref as cs ds 

displayDurDeref [] [] [] [] = do 
 putStrLn "------------------------------------------------------------------------------" -- Don't change to printf or compilation error will occur.
displayDurDeref ((a,b):[]) ((_,c):[]) ((_,d):[]) ((_,e):[])= do
 let c' = pc2mcs c        -- waiting durtion
     e' = pc2mcs e        -- conflict duration
     cmd = meanDuration c'  b -- mean duration of waiting
     emd = meanDuration e'  d -- mean duration of conflict
 printf "%-10d %-10.4f %-10.4f %-10.4f %-10.4f\n" a  c' cmd e' emd
 displayDurDeref [] [] [] []
displayDurDeref ((a,b):as) ((_,c):cs) ((_,d):ds)((_,e):es)  = do
 let c' = pc2mcs c        -- waiting durtion
     e' = pc2mcs e        -- conflict duration
     cmd = meanDuration c'  b -- mean duration of waiting
     emd = meanDuration e'  d -- mean duration of conflict
 printf "%-10d %-10.4f %-10.4f %-10.4f %-10.4f\n" a  c' cmd e' emd
 displayDurDeref as cs ds es


displayGroupDeref [] []= do
 putStrLn "------------------------------------------------------------------------------"
displayGroupDeref((a,b):[]) ((_,c):[])= do 
 printf "%-15d %-20d %-20d \n" a b c
 displayGroupDeref [] []
displayGroupDeref ((a,b):ys) ((_,c):zs) = do 
 printf "%-15d %-20d %-20d \n" a b c
 displayGroupDeref ys zs



displayFree [] [] [] = putStrLn "------------------------------------------------------------------------------"

displayFree ((a,b):[]) ((_,c):[]) ((_,d):[]) = do 
 let -- a  scheduler id
     -- b  total enters
     -- c  total cause in waiting
     -- d  total involve in conflicts
 ...
 printf "%-10d %-10d %-10d %-10.2f %-10d %-10.2f\n" a b c waitPercentage d confPercentage
 displayFree [] [] []
displayFree ((a,b):as) ((_,c):cs) ((_,d):ds)  = do 
 let -- a  scheduler id
     -- b  total enters
     -- c  total cause in waiting
     -- d  total involve in conflicts
     ...
     confPercentage  = percentage  b d
 printf "%-10d %-10d %-10d %-10.2f %-10d %-10.2f\n" a b c waitPercentage d confPercentage
 displayFree as cs ds 

displayDurFree [] [] [] [] = do 
 putStrLn "------------------------------------------------------------------------------" -- Don't change to printf or compilation error will occur.
displayDurFree ((a,b):[]) ((_,c):[]) ((_,d):[]) ((_,e):[])= do
 let c' = pc2mcs c        -- waiting durtion
     e' = pc2mcs e        -- conflict duration
     cmd = meanDuration c'  b -- mean duration of waiting
     emd = meanDuration e'  d -- mean duration of conflict
 printf "%-10d %-10.4f %-10.4f %-10.4f %-10.4f\n" a  c' cmd e' emd
 displayDurFree [] [] [] []
displayDurFree ((a,b):as) ((_,c):cs) ((_,d):ds)((_,e):es)  = do
 let c' = pc2mcs c        -- waiting durtion
     e' = pc2mcs e        -- conflict duration
     cmd = meanDuration c'  b -- mean duration of waiting
     emd = meanDuration e'  d -- mean duration of conflict
 printf "%-10d %-10.4f %-10.4f %-10.4f %-10.4f\n" a  c' cmd e' emd
 displayDurFree as cs ds es

displayGroupFree [] []= do
 putStrLn "------------------------------------------------------------------------------"
displayGroupFree((a,b):[]) ((_,c):[])= do 
 printf "%-15d %-20d %-20d \n" a b c
 displayGroupFree [] []
displayGroupFree ((a,b):ys) ((_,c):zs) = do 
 printf "%-15d %-20d %-20d \n" a b c
 displayGroupFree ys zs