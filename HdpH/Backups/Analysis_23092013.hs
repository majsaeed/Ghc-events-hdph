...
groupConflictByType gacc facc macc [] = [("AllGlob", gacc), ("AllFree", facc), ("Glob & Free", macc)]
...
-- no need for these function any more your can delete them
-- -- this fucntion counts how many times all the events in a conflict of type
-- -- globalise a GRef
-- countAllGlobOccurance :: Int -> [[HdpHEvent]] -> Int
-- countAllGlobOccurance acc [] = acc
-- countAllGlobOccurance acc (x:xs)
--  | allGlob x = countAllGlobOccurance (acc + 1) xs
--  | otherwise = countAllGlobOccurance acc xs

-- -- this function counts how many times all the events in a conflict of type
-- -- free a GRef
-- countAllFreeOccurance :: Int -> [[HdpHEvent]] -> Int
-- countAllFreeOccurance acc [] = acc
-- countAllFreeOccurance acc (x:xs)
--  | allFree x = countAllFreeOccurance (acc + 1) xs
--  | otherwise = countAllFreeOccurance acc xs
...
--   let     
  ...
  printf "%-15s %-20s\n" "No.Schedulers" "Wait. Occurance "
  displayGroupReg groupWaitReg 
  
  putStr "Total conflicts " 
  putStrLn (show totalConflicts )
  printf "%-30s %-20s\n" "Events type" "Conf. Occurance "
  displayGroupConfType groupConfType

  -- putStrLn "Registry Analysis for Globalising Global Reference Events"
  -- putStrLn "------------------------------------------------------------------------------"
  -- printf "%-10s %-10s %-10s %-10s\n" "SID" "Enter" "Wait" "Wait%"    
  -- displayReg  totalSchedGlob waitGlob 
  -- putStrLn "Displayed times are in microseconds."
  -- printf "%-10s %-20s %-10s\n" "SID" "Wait Duration" "Mean" 
  -- displayDurReg waitGlob  durWaitGlob 
  -- putStr "Max waiting duration : "
  -- printf "%07.5f\n" ( pc2mcs (maxDuration  0 (waitingEvents  pairGlob)))
  -- putStrLn "------------------------------------------------------------------------------"

  -- putStrLn "Number of times waitng  occured with this number of schedulers"
  -- printf "%-15s %-20s\n" "No.Schedulers" "Wait. Occurance " 
  -- displayGroupReg groupWaitGlob
  ...
  -- printf "%-15s %-20s\n" "No.Schedulers" "Wait. Occurance "
  -- displayGroupReg groupWaitDeref 
  ...
  -- putStrLn "------------------------------------------------------------------------------"
  -- printf "%-10s %-10s %-10s %-10s\n" "SID" "Enter" "Wait" "Wait%"
  -- displayReg  totalSchedFree waitFree 
  -- putStrLn "Displayed times are in microseconds."
  -- printf "%-10s %-20s %-10s\n" "SID" "Wait Duration" "Mean" 
  -- displayDurReg waitFree  durWaitFree
  -- putStr "Max waiting duration : "
...
displayGroupReg [] = do
 putStrLn "------------------------------------------------------------------------------"
displayGroupReg((a,b):[]) = do 
 printf "%-15d %-20d \n" a b 
 displayGroupReg [] 
displayGroupReg ((a,b):ys) = do 
 printf "%-15d %-20d \n" a b 
 displayGroupReg ys 

displayGroupConfType [] = do
 putStrLn "------------------------------------------------------------------------------"
displayGroupConfType ((a,b):[]) = do 
 printf "%-30s %-20d \n" a b 
 displayGroupConfType []
displayGroupConfType  ((a,b):ys) =  do
 printf "%-30s %-20d \n" a b 
 displayGroupConfType ys